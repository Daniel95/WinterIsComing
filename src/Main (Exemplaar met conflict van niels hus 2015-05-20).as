	...
	public class Main extends Sprite 
	{
		...
		private function collisions():void
		{
			...
			for (var y:int = 0; y < enemies.length; y++ )
			{
				...
				if (tree.hitTestObject(enemies[y]))
				{
					enemies[y].wallCol(tree.x, tree.y,tree.width,tree.height);
				}
			...
			{
				player.wallCol(tree.x, tree.y,tree.width,tree.height);
			}
			...
			for (var i:int = 0; i < arrows.length; i++ )
			{
				...
				if (arrows[i].hitTestObject(tree))
				{
					toRemove = true;
				}
		...
		private function createArrows(e:ShootEvent):void 
		{
			if (player != null)
			{
				...
				if (e.shooter.name == "instance9")//if player shoots. if this doesnt work, the instance name is probably not right, use trace(e.shooter.name);
				{
